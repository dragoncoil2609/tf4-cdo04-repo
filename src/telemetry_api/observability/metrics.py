...
# CDO-W12-020 Metrics
_amp_delivery_attempt_total: int = 0
_amp_delivery_retry_total: int = 0
_amp_delivery_failed_total: int = 0
_s3_failure_buffer_write_total: int = 0
_s3_failure_buffer_failed_total: int = 0
_ingest_buffered_total: int = 0
_failure_buffer_oldest_object_age_seconds: float = 0.0
...
# CDO-W12-020 metric recording functions
def record_amp_delivery_attempt() -> None:
    """Ghi nhận một lần thử gửi dữ liệu tới AMP."""
    global _amp_delivery_attempt_total
    _amp_delivery_attempt_total += 1


def record_amp_delivery_retry() -> None:
    """Ghi nhận một lần retry gửi dữ liệu tới AMP."""
    global _amp_delivery_retry_total
    _amp_delivery_retry_total += 1


def record_amp_delivery_failed() -> None:
    """Ghi nhận một lần gửi dữ liệu tới AMP thất bại hoàn toàn."""
    global _amp_delivery_failed_total
    _amp_delivery_failed_total += 1


def record_s3_failure_buffer_write() -> None:
    """Ghi nhận ghi S3 failure buffer thành công."""
    global _s3_failure_buffer_write_total
    _s3_failure_buffer_write_total += 1


def record_s3_failure_buffer_failed() -> None:
    """Ghi nhận ghi S3 failure buffer thất bại."""
    global _s3_failure_buffer_failed_total
    _s3_failure_buffer_failed_total += 1


def record_ingest_buffered() -> None:
    """Ghi nhận ingest payload được chuyển vào buffer thành công."""
    global _ingest_buffered_total
    _ingest_buffered_total += 1


def set_failure_buffer_oldest_object_age(age: float) -> None:
    """Cập nhật tuổi của object lâu nhất trong failure buffer."""
    global _failure_buffer_oldest_object_age_seconds
    _failure_buffer_oldest_object_age_seconds = age
...
def get_metrics_snapshot() -> dict[str, Any]:
    ...
    return {
        ...
        "telemetry_amp_delivery_attempt_total": _amp_delivery_attempt_total,
        "telemetry_amp_delivery_retry_total": _amp_delivery_retry_total,
        "telemetry_amp_delivery_failed_total": _amp_delivery_failed_total,
        "telemetry_s3_failure_buffer_write_total": _s3_failure_buffer_write_total,
        "telemetry_s3_failure_buffer_failed_total": _s3_failure_buffer_failed_total,
        "telemetry_ingest_buffered_total": _ingest_buffered_total,
        "telemetry_failure_buffer_oldest_object_age_seconds": _failure_buffer_oldest_object_age_seconds,
...
def reset_metrics_for_tests() -> None:
    ...
    global _amp_delivery_attempt_total, _amp_delivery_retry_total, _amp_delivery_failed_total
    global _s3_failure_buffer_write_total, _s3_failure_buffer_failed_total, _ingest_buffered_total
    global _failure_buffer_oldest_object_age_seconds
    ...
    _rejected_by_reason = {}
    _amp_delivery_attempt_total = 0
    _amp_delivery_retry_total = 0
    _amp_delivery_failed_total = 0
    _s3_failure_buffer_write_total = 0
    _s3_failure_buffer_failed_total = 0
    _ingest_buffered_total = 0
    _failure_buffer_oldest_object_age_seconds = 0.0
