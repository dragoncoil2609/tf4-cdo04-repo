...
| API Gateway HTTP API request | **$1.00 / 1M requests** first 300M requests/month |
| HTTP API VPC Link | **$0/hour, $0/request**; no separate VPC Link charge in this design |
...
AMP service charge cho demo scope vẫn ~`$0.00/month` trong estimate vì volume này nằm trong low/free-tier estimate đã ghi ở `05_cost_analysis.md`. Phần cần quyết định ở đây là **network path tới AMP**.
...
| S3 failure buffer/evidence/ALB logs | S3 | S3 Gateway Endpoint | **low/small** | Failure buffer, AI baseline, evidence export, ALB logs | S3 Gateway Endpoint |
| DynamoDB audit/policy | DynamoDB | DynamoDB Gateway Endpoint | **<0.10** | 25,920 audit writes + policy reads | DynamoDB Gateway Endpoint |
| Worker → AI Engine Path A | API Gateway HTTP API execute-api + VPC Link to ALB | NAT for Worker → `execute-api`; VPC Link for API Gateway → ALB | **~3.11** | 25,920 POST requests/month × ~120KB/request full 120p window; detailed request-cost math in §13 | NAT + API Gateway VPC Link |
...
Worker -> API Gateway AI    = 25,920 × ~0.00012GB = 3.11 GB/month
-------------------------------------------------------------
Subtotal                    = 12.96 GB/month
Planning rounded model      = 13.00 GB/month
```

Tài liệu này dùng **13 GB/month** để reflect Path A sau thay đổi mà vẫn không under-estimate.
...
Không đưa vào no-NAT MVP: `events`, `ecs`, `ecs-agent`, `ecs-telemetry`, `ssm`, `ssmmessages`, `ec2messages`, `xray`, trừ khi architecture bật thêm ECS Exec, SSM Parameter Store, X-Ray hoặc runtime gọi EventBridge APIs trực tiếp.
...
### 5.1 Current decision: 1 zonal NAT + S3/DynamoDB Gateway Endpoints
...
```text
$32.85 + 13 × $0.045 = $33.435 ~= $33.44/month
```
...
```text
$65.70 + 13 × $0.045 = $66.285 ~= $66.29/month
```
...
```text
$146.00 + 13 × $0.010 = $146.13/month
```

Nếu thêm `aps` control-plane endpoint:

```text
11 endpoints × 2 AZ × 730 × $0.010 + 13 × $0.010
= $160.60 + $0.13
= $160.73/month
```
...
```text
Hybrid delta vs current NAT decision
...
| Worker → AI Engine Path A | API Gateway HTTP API execute-api | N/A for `execute-api`; keep NAT, because no Interface VPCE for default execute-api endpoint | **3.11** | `$0.14` NAT data + `$0.03` API Gateway requests = **$0.17** | N/A | N/A | **Giữ NAT + HTTP API**; VPC Link từ API Gateway vào ALB không tính NAT/VPCE |
...
|---|---:|---:|---:|
| Current: 1 zonal NAT + S3/DDB Gateway EP | `$32.85 + 12×$0.045` | **$33.39** | Current decision |
...
Current decision dùng 1 NAT Gateway trong 1 AZ, trong khi ECS tasks chạy 2 AZ. Nếu task ở AZ còn lại route qua NAT ở AZ đặt NAT, có thể phát sinh cross-AZ data transfer charge và NAT không HA theo AZ.
...
3. Platform estimate trong `05_cost_analysis.md` là **~$158.16/month**, và với 20% ops buffer là **~$189.79/month**. Buffer còn khoảng **$10.21** trước hard budget `$200/month`, nên thêm AMP VPCE **+$14.50/month** đã có thể phá buffer nếu không giảm chỗ khác.
...
### 13.1 Pricing facts dùng cho phép tính

AWS Pricing API tại `us-east-1` trả về:

| Item | Pricing API SKU / usage type | Unit price |
|---|---|---:|
| API Gateway HTTP API request | `AmazonApiGateway`, `USE1-ApiGatewayHttpRequest`, `ApiGatewayHttpApi` | **$0.000001/request** = $1.00 / 1M requests, first 300M requests/month |
| NAT Gateway data processing | `AmazonEC2`, `NatGateway-Bytes`, `NatGateway` | **$0.045/GB** |
| NAT Gateway hourly | `AmazonEC2`, `NatGateway-Hours`, `NatGateway` | **$0.045/hour** |
| API Gateway HTTP API VPC Link | HTTP API private integration to ALB listener | **$0 extra hourly/request in this design** |

Không cộng lại NAT hourly, ALB hourly, ECS Fargate hourly, CloudWatch logs base cost hoặc API Gateway VPC Link fixed cost vào per-request math. Các phần đó đã nằm trong workload monthly baseline; request math chỉ tính incremental Path A traffic:
...
Một POST Worker → AI Engine đại diện một service trong full 120p window:

```text
7 metrics × 120 one-minute samples = 840 sample points/request
```

Payload size chưa có field-by-field byte measurement trong docs, nên dùng estimate compact JSON:

```text
midpoint payload = 840 points × 150 bytes/point + ~4KB envelope/response/headers
                 ~= 130,000 bytes/request
                 ~= 0.00012 GB/request
```

Sensitivity:

| Assumption | Bytes/point | Estimated GB/request |
|---|---:|---:|
| Low compact JSON | 100 | ~0.00008 GB |
| Midpoint planning | 150 | ~0.00012 GB |
| High compact JSON | 200 | ~0.00016 GB |

### 13.3 Cost cho một POST request full 120p window

Midpoint:

```text
API Gateway request cost/request = $1.00 / 1,000,000
                                 = $0.00000100

NAT data cost/request = 0.00012 GB × $0.045/GB
                      = $0.00000540

Total incremental Path A cost/request
= $0.00000100 + $0.00000540
= $0.00000640/request
```

Vậy **một POST Worker → AI Engine cho full 120p window tốn khoảng $0.0000064**, tức khoảng **0.00064 cent/request**.

### 13.4 Monthly request count và monthly cost

Prediction cadence hiện tại:

```text
3 services × 12 cycles/hour × 24 hours/day × 30 days
= 25,920 POST requests/month
```

Monthly cost:

```text
API Gateway monthly = 25,920 × $0.00000100
                    = $0.02592 ~= $0.03/month

NAT data monthly = 25,920 × $0.00000540
                 = $0.139968 ~= $0.14/month

Total Path A incremental request monthly
= $0.03 + $0.14
= $0.17/month
```
...
| Payload model | Per-request total | 25,920 requests/month |
|---|---:|---:|
| 100 bytes/point | ~$0.0000048 | ~$0.13/month |
| 150 bytes/point midpoint | **~$0.0000064** | **~$0.17/month** |
| 200 bytes/point | ~$0.0000082 | ~$0.21/month |

### 13.5 Reflect vào workload total sau Path A

`05_cost_analysis.md` đã có API Gateway row khoảng **$0.03/month** nhưng chưa tách riêng NAT data cho Worker → public `execute-api` payload. Refined request math nên ghi:

```text
API Gateway HTTP API request charge      ~= $0.03/month
Worker -> execute-api NAT data charge    ~= $0.14/month
Total Path A request variable charge     ~= $0.17/month
```

Nếu cộng vào workload hiện tại:

```text
Current Path A baseline in 05_cost_analysis.md = ~$158.19/month
Replace API Gateway-only $0.03 with refined $0.17
Delta = +$0.14/month
Refined monthly workload = $158.19 + $0.14 = $158.33/month
20% ops buffer = $158.33 × 1.20 = $190.00/month
```

Kết luận: Path A request variable cost **không material** với budget. Nó tăng estimate khoảng **$0.14/month** so với row API Gateway-only hiện tại và vẫn giữ workload dưới hard cap **$200/month** sau 20% buffer.
