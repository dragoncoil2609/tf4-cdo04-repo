...
| Cổng ingress public | 1× Application Load Balancer | Public ALB chỉ nhận telemetry `/v1/ingest`. Sandbox hiện dùng HTTP tạm để đưa hệ thống chạy trước; non-sandbox/final hardening dùng ACM certificate ARN tại `us-east-1` để bật HTTPS. Terraform phải yêu cầu `allowed_ingress_cidrs` rõ ràng; không dùng default mở `0.0.0.0/0`. Luồng Worker → AI dùng ECS Service Connect trong private subnets; thiết kế không cần Route 53/private DNS cho Luồng AI. | **$22.27/tháng tại `us-east-1`** = ALB $16.43 + 1 LCU trung bình $5.84. |
