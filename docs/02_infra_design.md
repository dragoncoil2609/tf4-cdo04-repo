...
|---|---|---|---|
| Tầng compute | ECS Fargate Linux/x86 | Chạy 1 task cho Telemetry API, 1 task cho Prediction Worker và 2 task cho AI Engine trong cùng ECS cluster. Fargate phù hợp vì không phải quản lý EC2, có task role riêng, và chạy được API, worker, AI serving theo cùng mô hình container/private subnet. | **$90.10/tháng tại `us-east-1`** = 4 task × 730h × (1 vCPU/2GB telemetry + 0.5 vCPU/1GB worker + 2×0.5 vCPU/1GB AI). |
| Cổng ingress qua API Gateway + ALB nội bộ | 1× Application Load Balancer | API Gateway HTTP API là public front door cho `/health`, `/v1/ingest`, `/v1/predict`; ALB chuyển thành internal HTTP `:80`, chỉ nhận từ API Gateway VPC Link SG và forward vào ECS target groups. | **$22.27/tháng tại `us-east-1`** = existing ALB $16.43 + 1 LCU trung bình $5.84. Không thêm ALB thứ hai. |
| Kho metric TSDB | Amazon Managed Service for Prometheus (AMP) | AMP workspace lưu metric dạng Prometheus samples/labels. Terraform v1 dùng ADOT Collector sidecar colocated trong telemetry-api task, scrape `localhost:8080/metrics` rồi remote_write vào AMP bằng SigV4; app-direct remote_write chỉ là fallback kỹ thuật nếu app implement protobuf/Snappy/SigV4/retry đầy đủ. Prediction Worker query PromQL `query_range` đủ 120 phút theo tenant/service/metric trước khi gọi AI. | **~$0.00/tháng ở demo scope**: 907,200 samples/month và ~21.8M query samples/month gần bằng 0 nhưng query samples vẫn billed theo usage. AMP không có fixed `db.influx.medium` instance-hour cost. |
...
| API Gateway SigV4 front door | API Gateway HTTP API + VPC Link | API Gateway enforces `AWS_IAM`/SigV4 for `POST /v1/ingest` and `POST /v1/predict`; `GET /health` stays `NONE` for smoke. Worker egress to execute-api uses existing NAT. API Gateway private integration to ALB uses VPC Link, not NAT. | **~$0.17/tháng** at 25,920 AI POST calls/month: ~$0.03 HTTP API request charge + ~$0.14 NAT data for full 120m payload. |
| **Tổng chi phí** |  | Network path dùng 1 zonal NAT + S3/DynamoDB Gateway Endpoints; TSDB dùng AMP; Worker → AI dùng API Gateway HTTP API + VPC Link + internal ALB; Service Connect giữ làm rollback/fallback. | **~$158.33/tháng tại `us-east-1`** cho full always-on x86 design. Thêm 20% buffer vận hành là **~$190.00/tháng**, vẫn dưới hard budget $200. |
...
2. expose `/metrics` (Prometheus text format); ADOT sidecar trong cùng task scrape `localhost:8080/metrics`, rồi `remote_write` Prometheus samples/labels vào AMP bằng SigV4;
...
|---|---|---|
| Budget fit | **~$158.33/tháng** full always-on x86 design tại `us-east-1` với API Gateway public front door + internal ALB + ECS Service Connect rollback/fallback cho Worker → AI; **~$189.79** với 20% buffer vẫn dưới hard budget $200. ADOT Collector sidecar chạy cùng telemetry-api task (1024/2048, upsized for single-writer MVP), scrape `localhost:8080/metrics`. | Dashboard-only có thể rẻ hơn, nhưng không cover đủ prediction workflow, audit-per-call, fallback, evidence-link requirement và AI serving nội bộ |
...
Điểm cost của thiết kế này không phải là rẻ nhất tuyệt đối. Rẻ nhất tuyệt đối sẽ là dashboard-only hoặc vài CloudWatch alarm tĩnh, nhưng hai hướng đó không giải quyết đúng pain point client đã nêu: không có người nhìn dashboard 24/7 và threshold tĩnh dễ quá nhạy hoặc quá trễ. Vì vậy tiêu chí tối ưu là **cost-to-requirement coverage**. Sau khi chuyển sang AMP tại `us-east-1` và dùng API Gateway HTTP API `AWS_IAM` cho Worker → AI Path A (với internal ALB sau VPC Link, ECS Service Connect rollback/fallback), full always-on x86 design fit hard budget $200 cả trước và sau 20% buffer với ADOT Collector sidecar colocated trong telemetry-api task (1024/2048, upsized for single-writer MVP). Dù tối ưu ở đâu, **không tắt audit/fallback**.
...
- **Auth rule**: SigV4 + `X-Tenant-Ingest-Token` là production auth path; legacy `Authorization: Bearer` chỉ là local/internal fallback. `X-Tenant-Id` chỉ là context header sau khi credential đã được verify.
...
- Worker gọi AI qua **Path A**: private subnet -> NAT Gateway -> API Gateway HTTP API `execute-api` endpoint với `AWS_IAM`/SigV4 -> VPC Link -> internal ALB listener `:80` -> AI Engine target group -> AI Engine task `:8080`. ECS Service Connect được giữ làm migration rollback/fallback, không còn là primary auth path. Nền tảng không cần Route 53/private DNS cho Luồng AI; Worker lấy endpoint config/timeout từ Secrets Manager/SSM config.
- Auth giữa Worker và AI dùng **IAM SigV4** enforce tại API Gateway HTTP API `AWS_IAM`. Worker ký request SigV4 service `execute-api` và gửi tới API Gateway endpoint. API Gateway xác thực signature, nếu không hợp lệ trả `403` trước khi request tới internal ALB. ECS Service Connect chỉ giữ làm rollback/fallback migration path. AI Engine app-side SigV4 verification là production hardening, không nằm trong MVP. Không dùng API key/service token làm auth chính.
...
- API Gateway HTTP API là public front door; ALB là internal chỉ nhận traffic từ API Gateway VPC Link. Worker gọi AI bằng API Gateway HTTP API `AWS_IAM` -> VPC Link -> internal ALB `:80` -> AI Engine target group; ECS Service Connect giữ làm rollback/fallback.
- Internal ALB target group type `ip`; AI Engine health check dùng ECS container/service health check đúng Deployment Contract: path `/health`, port `8080`, interval 30 giây, healthy threshold 2 consecutive 200, unhealthy threshold 3 consecutive non-200.
...
|---|---|---|---|
| Telemetry API | 1 task × 1 vCPU / 2GB | N/A (single-writer MVP — no autoscaling) | N/A — pinned at 1 |
| Prediction Worker | 1 task × 0.5 vCPU / 1GB | Queue age >2 phút, visible messages >20 trong 5 phút, worker timeout | Max 5 task |
| AI Engine | 2 task × 0.5 vCPU / 1GB | CPU >70% (target tracking) và AI p95 >350ms/p99 >500ms (latency step scaling) drive scale-out; 5xx/RPS alarms là SNS-only, không phải scaling actions trừ khi được wire rõ | Max 4 task theo AI Deployment Contract |
...
|---|---|---|
| Internal ALB SG | Sandbox: 80 từ explicit VPC Link SG ingress; non-sandbox/future: 443 với ACM và 80 redirect HTTPS | ECS API SG (ingest path) |
...
| Mất một AZ | ALB target unhealthy hoặc ECS thiếu task ở 1 AZ | ALB/Service Connect route sang task khỏe ở AZ còn lại; ECS chạy task thay thế ở subnet khỏe | < 5min | < 1min |
