...
| Cổng ingress public + AI restricted listener | 1× Application Load Balancer | Public ALB `:443` chỉ nhận telemetry `/health` và `/v1/ingest`; `/v1/predict` không public. Same ALB có restricted listener `:8443` chỉ cho API Gateway VPC Link SG để forward vào AI Engine target group. | **$22.27/tháng tại `us-east-1`** = existing ALB $16.43 + 1 LCU trung bình $5.84. Không thêm ALB thứ hai. |
...
| ECS Fargate + public ALB + Service Connect | Chạy API, worker và AI serving bằng container, không quản lý EC2, hỗ trợ private subnet, task role, autoscaling, public ingest boundary và private service-to-service discovery/load balancing. | Có fixed cost cho public ALB và task chạy nền; Service Connect proxy cần CPU/memory headroom trong task. | ✅ **Chọn.** Đúng yêu cầu đề bài, match yêu cầu AI host trong ECS cluster và đủ production-like cho fintech workload. |
