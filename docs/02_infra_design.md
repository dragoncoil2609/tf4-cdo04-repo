...
|---|---|---|
| Budget fit | **~$158.16/tháng** full always-on x86 design tại `us-east-1` với public ingest ALB + ECS Service Connect cho Worker → AI; **~$189.79** với 20% buffer vẫn dưới hard budget $200. ADOT Collector sidecar chạy cùng telemetry-api task (1024/2048, upsized for single-writer MVP), scrape `localhost:8080/metrics`. | Dashboard-only có thể rẻ hơn, nhưng không cover đủ prediction workflow, audit-per-call, fallback, evidence-link requirement và AI serving nội bộ |
