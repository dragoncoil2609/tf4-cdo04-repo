...
`contracts/deployment-contract.md` mô tả internal ALB/private DNS cho AI Engine. CDO Terraform v1 không tạo internal ALB cho AI; Worker gọi AI qua ECS Service Connect service name trong private subnets. Quyết định này giữ nguyên ý định của contract: AI endpoint private, không public, chỉ Worker được phép gọi. Không sửa contract file.
...
| 4 | Ingress + private service routing | Public `/v1/ingest` đi qua public ALB; Worker gọi AI qua ECS Service Connect service name trong private subnets. Route 53/private DNS không nằm trong MVP. |
...
- **AI Engine trong Terraform v1**: dùng ECS rolling deployment circuit breaker giống API/Worker. ECS Service Connect xử lý private service discovery/load balancing; SigV4 enforcement phải nằm ở AI Engine middleware/sidecar trong W12 final.
- **Post-MVP hardening**: nếu cần canary/blue-green sau này, ưu tiên ECS-native blue/green với Service Connect. CodeDeploy chỉ dùng khi thiết kế thêm target group/listener/test traffic rõ ràng.
...
- **Target RTO (Recovery Time Objective)**: platform service rollback theo ECS steady-state/circuit-breaker timing. Target trong contract `<60s` vẫn là mục tiêu rollback mong muốn cho AI, nhưng Terraform v1 không implement CodeDeploy canary.
