...
`contracts/deployment-contract.md` mô tả internal ALB/private DNS cho AI Engine. CDO Terraform Path A không tạo ALB thứ hai; Worker gọi AI qua API Gateway HTTP API `AWS_IAM`, API Gateway dùng VPC Link tới same existing ALB restricted listener `:8443`, rồi forward tới AI Engine target group. Worker ở private subnet ra API Gateway bằng NAT Gateway hiện có. ECS Service Connect được giữ làm rollback/fallback trong migration. Không sửa contract file.
...
| 4 | Ingress + private AI routing | Public `/v1/ingest` đi qua public ALB `:443`; Worker gọi AI qua NAT → API Gateway HTTP API `AWS_IAM` → VPC Link → same ALB restricted listener `:8443` → AI Engine target group. ECS Service Connect giữ làm rollback/fallback. |
...
- **AI Engine trong Terraform v1**: dùng ECS rolling deployment circuit breaker giống API/Worker. API Gateway HTTP API enforce `AWS_IAM`/SigV4; VPC Link + same ALB restricted listener `:8443` xử lý private routing vào AI target group. ECS Service Connect giữ làm rollback/fallback trong migration.
