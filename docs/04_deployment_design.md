...
│   │   ├── data/          # Timestream for InfluxDB db.influx.medium, DynamoDB audit/policy, SQS/DLQ, S3 evidence, Secrets/KMS
│   │   ├── compute/       # ALB, ECS Cluster, ECS Fargate Services (Telemetry API, Prediction Worker, AI Engine), Scheduler
...
| 4 | Load Balancing (public ALB + internal AI ALB) | Public `/v1/ingest` đi qua public ALB; Worker gọi AI qua internal ALB DNS. Route 53/private DNS không nằm trong baseline MVP. |
...
| Metric Evidence | Amazon Timestream for InfluxDB | Source of truth cho telemetry window 120 phút; Prediction Worker query bằng Flux theo tenant/service trước khi gọi AI. |
...
- [x] Q2: Có cần cấu hình CloudWatch Logs Subscription Filter/Kinesis Firehose/Athena cho baseline không? - *Resolved: Không dùng trong baseline; CloudWatch + Timestream for InfluxDB + DynamoDB là primary observability/evidence path.*
...
- [`08_adrs.md`](08_adrs.md) - Lưu vết lý do chọn SLO Early-Warning Control Plane, ECS Fargate, Timestream for InfluxDB, DynamoDB audit, EventBridge/SQS orchestration và AI Engine on ECS.
