...
*   **Bước 1: Thu thập số liệu (Telemetry Ingestion)**
    Telemetry Ingestion API nhận dữ liệu metric (CPU, latency, connection utilization, queue depth,...) từ 3 dịch vụ demo và ghi vào Amazon Managed Service for Prometheus (AMP).
...
*   **Bước 3: Lấy dữ liệu & Gọi AI (Prediction Worker)**
    Prediction Worker nhận job từ SQS, thực hiện truy vấn dữ liệu metric lịch sử từ 1 đến 2 giờ gần nhất trong AMP bằng PromQL làm dữ liệu đầu vào.
...
*   **Bước 6: Phát cảnh báo kèm Chứng cứ (SNS Alert & Evidence)**
    ...
    *   *Metric evidence*: Link tham chiếu PromQL/query_range trong AMP.
