...
3.  **Kiểm tra dịch vụ ECS**: Nếu chi phí tăng đột biến, kiểm tra `telemetry-api` đang giữ 1 task MVP single-writer và xem `ai-engine` / `prediction-worker` đã được scale về 0 hay chưa (nếu circuit breaker đã kích hoạt).
